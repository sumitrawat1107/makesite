...
if ( ! function_exists( 'woodmart_vc_extra_classes' ) ) {
	...
	function woodmart_vc_extra_classes( $class, $base, $atts ) {
		...
		if ( ! empty( $atts['wd_animation'] ) ) {
			...
			if ( ! empty( $atts['wd_animation_duration'] ) ) {
				$class .= ' wd-animation-' . $atts['wd_animation_duration'];
			}
...
if ( ! function_exists( 'woodmart_vc_map_shortcodes' ) ) {
	...
	function woodmart_vc_map_shortcodes() {
		...
		vc_add_param(
			'vc_single_image',
			woodmart_get_vc_animation_map()
		);

		vc_add_param(
			'vc_single_image',
			woodmart_get_vc_animation_delay_map()
		);

		vc_add_param(
			'vc_single_image',
			woodmart_get_vc_animation_duration_map()
		);
...
if ( ! function_exists( 'woodmart_get_vc_animation_map' ) ) {
	 ...
	 * @return array
	 */
	function woodmart_get_vc_animation_map() {
		return array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Animation', 'woodmart' ),
			'group'       => esc_html__( 'Animations', 'woodmart' ),
...
if ( ! function_exists( 'woodmart_get_vc_animation_delay_map' ) ) {
	/**
	 * Get animation delay map settings for VC.
	 *
	 * @return array
	 */
	function woodmart_get_vc_animation_delay_map() {
		return array(
			'type'       => 'textfield',
			'heading'    => esc_html__( 'Animation Delay (ms)', 'woodmart' ),
			'group'      => esc_html__( 'Animations', 'woodmart' ),
...
if ( ! function_exists( 'woodmart_get_vc_animation_duration_map' ) ) {
	/**
	 * Get animation duration map settings for VC.
	 *
	 * @return array
	 */
	function woodmart_get_vc_animation_duration_map() {
		return array(
			'type'       => 'dropdown',
			'heading'    => esc_html__( 'Animation duration', 'woodmart' ),
			'group'      => esc_html__( 'Animations', 'woodmart' ),
...
if ( ! function_exists( 'woodmart_get_vc_animations_css_classes' ) ) {
	/**
	 ...
	 * @return string
	 */
	function woodmart_get_vc_animations_css_classes( $data ) {
		$classes = '';

		if ( ! empty( $data['wd_animation'] ) ) {
			woodmart_enqueue_js_library( 'waypoints' );
			woodmart_enqueue_js_script( 'animations' );
			woodmart_enqueue_inline_style( 'animations' );

			$duration = ! empty( $data['wd_animation_duration'] ) ? $data['wd_animation_duration'] : 'normal';

			$classes .= ' wd-animation-' . $data['wd_animation'];
			$classes .= ' wd-animation-' . $duration;

			if ( ! empty( $data['wd_animation_delay'] ) ) {
				$classes .= ' wd_delay_' . $data['wd_animation_delay'];
			}
		}
