	...
	function woodmart_shortcode_brands( $atts, $content = '' ) {
		...
		if( ! is_wp_error( $brands ) && count( $brands ) > 0 ) {
			foreach ($brands as $key => $brand) {
				...
				} elseif ( apply_filters( 'woodmart_brands_element_images_with_attributes', true ) && $url_to_id ) {
					echo wp_get_attachment_image( $url_to_id, 'full', false, array( 'title' => $brand->name, 'alt' => $brand->name ) );
				} else {
